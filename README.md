...
> [!IMPORTANT]
> 软件会同时输出从动件中心的理论轮廓和考虑从动件半径的实际二维包络。更多细节请看[详细讲解视频以及下面介绍](assets/screenshots/app-overview.png)
...
1. 导入 `examples/waypoints.txt` 或自己的打卡点文件。
...
“直线-圆弧实验”会先生成相切直线与圆弧，再按 `L、m` 对轨迹做 0～6 级前后轮变换。级数越高不代表结果越好；建议从 1 级开始比较。该模式计算量高于平滑曲线模式，适合用于方案验证。

“圆链实验”会把可由同一圆弧覆盖的连续打卡点自动合并，并用两段共切圆弧连接每组起终方向；随后同样可以进行多级轮迹平滑。建议先使用 1 级和默认合并程度测试。
