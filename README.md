...
> [!IMPORTANT]
> 直圆结果需要搭配 [01铁匠网站](http://43.143.111.245:3030/) 的对应导入优化功能继续使用。该服务需要付费，但提供售后支持，当前配套流程较稳定；需要的可以加qq群：862127093了解。
...
circles.txt            # 精简相切圆参数：起点、圆心/带符号半径、终点
cam_xyz.txt                # 实际加工凸轮 X、Y、Z 坐标
cam_xyz_theoretical.txt    # 从动件中心的理论凸轮 X、Y、Z 坐标
```

路径文件固定为“序号、X、Y、曲率、曲率半径、归一化距离”六列。直圆路径可单独导出精简相切圆参数，文件头给出初始发车角度，后续为纯数值 `X Y R`；`R=0` 的首尾行分别是起点和终点，非零 `R` 为带方向的圆心半径。保存凸轮时，所选文件保存考虑从动件半径的实际包络，同目录自动生成带 `_theoretical` 后缀的理论轮廓；两者均为纯数值 `X Y Z`。选择“不补全”时没有完整一圈数据，路径页不会启用凸轮导出按钮。
